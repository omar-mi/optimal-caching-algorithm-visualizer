...
def next_step():
    ...
    if to_replace:
        ...
        for i in range(len(cache_array)):
            if cache_array[i] == to_replace[0]:
                cache_array[i] = to_replace[1]
        print(cache_array)
        to_replace = None
        canvas.delete("all")
        draw_cache_list(cache)
...
access_input.insert(0, 'ABBCCCDBBDDDDEB')


# ---------------- START ----------------
def on_start_viz():
    global cache, cache_array, access_sequence, cache_elements, access_sequence_elements
    ...
    cache_array = [c for c in cache_elements]
    print(cache_array)
